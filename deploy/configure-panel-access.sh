...
[[ "$PUBLIC_PORT" =~ ^[0-9]+$ ]] && (( PUBLIC_PORT >= 1 && PUBLIC_PORT <= 65535 )) || fail "invalid panel port"
...
case "$PUBLIC_PORT" in
  22|80|443|585|18080)
    fail "TCP port ${PUBLIC_PORT} is reserved. Recommended SG-AWG-Panel port: 62443"
    ;;
esac
(( PUBLIC_PORT >= 1024 )) || fail "panel port must be between 1024 and 65535"
...
install -d -m 0755 "$ACME_ROOT/.well-known/acme-challenge" "$PLACEHOLDER_ROOT"
