...
if command -v ip >/dev/null 2>&1; then
  for interface in awg0 $(ip -o link show 2>/dev/null | awk -F': ' '$2 ~ /^sgo[0-9]+(@.*)?$/ {sub(/@.*/, "", $2); print $2}'); do
...
if (( PURGE_AWG == 1 )); then
  ...
  echo "[SG-AWG-Panel] [OK] Данные и службы SG-AWG-Panel удалены. Пакет AmneziaWG оставлен установленным."
fi
