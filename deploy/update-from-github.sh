...
rollback(){
  ...
  if (( ROLLBACK_NEEDED )); then
    ...
    if (( SITE_EXISTED )); then cp -a "$BACKUP_DIR/sg-awg-panel.nginx" /etc/nginx/sites-available/sg-awg-panel; else rm -f /etc/nginx/sites-available/sg-awg-panel /etc/nginx/sites-enabled/sg-awg-panel; fi
    ...
    if (( NGINX_WAS_ACTIVE )); then nginx -t && systemctl restart nginx || true; else systemctl stop nginx 2>/dev/null || true; fi
...
if [[ -f "$DATA_DIR/panel.db" ]]; then SOURCE_DB="$DATA_DIR/panel.db" TARGET_DB="$BACKUP_DIR/panel.db" python3 - <<'PYDB'
import os,sqlite3
...
if [[ -f /etc/nginx/sites-available/sg-awg-panel ]]; then SITE_EXISTED=1; cp -a /etc/nginx/sites-available/sg-awg-panel "$BACKUP_DIR/sg-awg-panel.nginx"; fi
systemctl is-active --quiet nginx && NGINX_WAS_ACTIVE=1 || true
for unit in sg-awg-panel.service sg-awg-backup.service sg-awg-backup.timer sg-awg-recovery.service; do [[ -f "/etc/systemd/system/$unit" ]] && cp -a "/etc/systemd/system/$unit" "$BACKUP_DIR/$unit"; done
ROLLBACK_NEEDED=1
...
OLD_EMAIL="$(get_env AWGPANEL_HTTPS_EMAIL '')"
...
python3 - "$ENV_FILE" "$OLD_PORT" "$OLD_SCHEME" "$OLD_HOST" "$OLD_EMAIL" <<'PY'
from pathlib import Path
import sys
p=Path(sys.argv[1]); public_port,scheme,host,email=sys.argv[2:]
updates={'AWGPANEL_BIND_ADDRESS':'127.0.0.1','AWGPANEL_PORT':'18080','AWGPANEL_BACKEND_PORT':'18080','AWGPANEL_PUBLIC_PORT':public_port,'AWGPANEL_PUBLIC_SCHEME':scheme,'AWGPANEL_PUBLIC_HOST':host,'AWGPANEL_HTTPS_EMAIL':email,'AWGPANEL_TRUST_PROXY_HEADERS':'1','AWGPANEL_SECURE_COOKIES':'1' if scheme=='https' else '0'}
...
ACCESS_ARGS=(--scheme "$OLD_SCHEME" --port "$OLD_PORT")
[[ -n "$OLD_HOST" ]] && ACCESS_ARGS+=(--domain "$OLD_HOST")
[[ -n "$OLD_EMAIL" ]] && ACCESS_ARGS+=(--email "$OLD_EMAIL")
