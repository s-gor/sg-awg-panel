...
def test_207_release_identifiers():
    assert '__version__ = "0.7.0-RC3"' in read("awgpanel/__init__.py")
    assert 'version = "0.7.0rc3"' in read("pyproject.toml")
    assert 'RELEASE_VERSION="v0.7.0-RC3"' in read("install.sh")
    assert 'AGENT_VERSION = "0.7.0-RC3"' in read("node_agent/agent.py")
