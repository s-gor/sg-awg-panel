...
def test_self_extracting_installers_are_reproducible_and_do_not_need_unzip():
    ...
    assert "0.7.0-RC6-INSTALL-SG-AWG-PANEL.run" not in builder  # version is derived, not hardcoded
...
def test_readme_leads_with_no_unzip_installation():
    ...
    assert "0.7.0-RC6-INSTALL-SG-AWG-PANEL.run" in readme
    assert "0.7.0-RC6-INSTALL-SG-AWG-NODE.run" not in readme
    assert readme.count("sudo bash 0.7.0-RC6-INSTALL-SG-AWG-PANEL.run") >= 2
    assert "sudo bash 0.7.0-RC6-INSTALL-SG-AWG-PANEL.run" in readme
